...
The game is deployed and public. **The 3D aerial engine (`packages/game3d`) is
the one and only client** — there is no separate 2D build in production anymore.

| What | URL |
|------|-----|
| **Play** (3D game) | https://trampa-nu.vercel.app |
| Mirror (same 3D build) | https://trampa-3d.vercel.app |
| API | https://trampa-api.vercel.app |

The full online game — leagues, daily challenge, streamer rooms/tournaments,
ghosts, traps, saboteur ranking, anti-cheat — runs entirely on the deterministic
**3D** engine (`@trampa/shared` `sim3d`). The client is a static Vercel project
built from `packages/game3d`; `trampa` (primary URL) and `trampa-3d` (mirror)
...
- Hosting: **Vercel** team `hola-glow` — projects: `trampa` + `trampa-3d`
  (static 3D client, same build) and `trampa-api` (serverless function).
  Database: **Supabase** project `trampa-game` (org ViajeChina, region
  `eu-west-3`, ref `bncuwqwvxafhthlymxyd`), a fresh isolated project.

## Architecture of the deploy

- **Client** — the Vite build (`packages/game3d`) deployed as static files, built
...
```bash
# API (after code changes) — bundled esbuild function
npm run build:shared
npm run bundle:vercel --workspace @trampa/server
cd packages/server/vercel-out
vercel link --yes --project trampa-api --scope hola-glow   # re-link if .vercel is missing
vercel deploy --prod --yes --scope hola-glow               # DATABASE_URL is a project env var

# 3D client (after code changes) — must bake the API URL at build time
VITE_API_URL=https://trampa-api.vercel.app npm run build --workspace @trampa/game3d
cd packages/game3d/dist
vercel link --yes --project trampa   --scope hola-glow && vercel deploy --prod --yes --scope hola-glow
vercel link --yes --project trampa-3d --scope hola-glow && vercel deploy --prod --yes --scope hola-glow
```

> The `vercel-out/` and `dist/` folders are regenerated by the build, which wipes
> their `.vercel` link — always `vercel link --project <name>` before `deploy`,
> or a new project gets created from the folder name.

## Database migrations

Applied to Supabase via the migration SQL in `packages/server/src/migrations/`
(001 init, 002 notifications, 003 run_deaths, 004 rooms, **005 trap_float** —
widens `traps.slot_x/slot_y` to `double precision` for 3D float lane/z slots).
Tracked in the `_migrations` table. To apply new ones, run them against the
pooler connection string or via the Supabase SQL editor, then insert the
filename into `_migrations`.
